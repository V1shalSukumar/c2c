...
class AdvancedKineticCanvas:
    def __init__(self):
        ...
        print("- Hold up two fingers to open the menu")
    ...
    def is_two_fingers_up(self, hand_landmarks):
        """Check if two fingers (index and middle) are extended"""
        if not hand_landmarks:
            return False
    ...
    def draw_menu(self, frame):
        """Draws the in-window menu"""
        if not self.menu_visible:
            return

        menu_width, menu_height = 250, len(self.menu_options) * 50 + 20
        menu_x, menu_y = 10, 10
        ...
        for i, (label, callback) in enumerate(self.menu_options):
            option_x, option_y = menu_x + 10, menu_y + 40 + i * 50
            option_rect = (option_x, option_y - 30, menu_width - 20, 50)
            
            is_touching = False
            for finger_pos in self.current_extended_fingers.values():
                if (option_rect[0] <= finger_pos['pos'][0] <= option_rect[0] + option_rect[2] and
                    option_rect[1] <= finger_pos['pos'][1] <= option_rect[1] + option_rect[3]):
                    is_touching = True
                    break

            if is_touching:
                cv2.rectangle(frame, (option_rect[0], option_rect[1]),
                              (option_rect[0] + option_rect[2], option_rect[1] + option_rect[3]),
                              (0, 255, 255), -1)
                
                touch_id = f"menu_{i}"
                current_time = time.time()
                if touch_id not in self.menu_touch_tracker:
                    self.menu_touch_tracker[touch_id] = current_time
                
                if current_time - self.menu_touch_tracker[touch_id] > 0.5:
                    callback()
                    self.menu_touch_tracker.clear()
            else:
                if f"menu_{i}" in self.menu_touch_tracker:
                    del self.menu_touch_tracker[f"menu_{i}"]
            
            cv2.putText(frame, label, (option_x, option_y), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 2)
