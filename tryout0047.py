...
class VirtualIcon:
    ...
    def draw(self, frame, is_touching=False, touch_progress=0.0):
        ...
        # Base color - changes when touched
        if is_touching:
        ...
        if self.icon_type == "brush":
            ...
            # Draw minus icon for opacity down
            cv2.line(frame, (center_x - 8, center_y), 
                    (center_x + 8, center_y), color, 3)
        
        elif self.icon_type == "kaleidoscope":
            # Draw kaleidoscope icon (star-like pattern)
            points = []
            for i in range(8):
                angle = i * math.pi / 4
        ...
        # Draw activation progress bar if being touched
...
class AdvancedKineticCanvas:
    def __init__(self):
        ...
        # Virtual Icons System
        self.icons = []
        self.setup_virtual_icons()
        self.icon_touch_tracker = {}  # Track touch duration for each icon
    ...
    def setup_virtual_icons(self):
        ...
        for icon_type, label, index in icons_config:
            y_pos = margin + index * (icon_size + margin)
    ...
    def check_icon_touches(self, extended_fingers_all_hands):
        ...
        # Get all finger positions from all hands
        all_finger_positions = []
        for hand_fingers in extended_fingers_all_hands.values():
            all_finger_positions.append(hand_fingers['pos'])
        
        # Check each icon
        for icon in self.icons:
            is_being_touched = False
            
            # Check if any finger is touching this icon
            ...
            if is_being_touched:
                ...
                # Activate icon if touched long enough
    ...
    def draw_kaleidoscope_brush(self, start_pos, end_pos, color, velocity):
        ...
        # Calculate the vector from the center to the start position
        vec_x_start, vec_y_start = start_pos[0] - center_x, start_pos[1] - center_y
        
        # Calculate the vector from the center to the end position
        ...
        for i in range(self.kaleidoscope_segments):
            ...
            # Apply the selected brush to the rotated positions
            self.apply_brush(rotated_start_pos, rotated_end_pos, color, velocity)
            
            # Add a mirrored reflection
    ...
    def save_image(self):
        ...
        if self.background_enabled and self.background_image is not None:
            # Create a combined image with the drawing on top of the background
    ...
    def start_video_recording(self):
        ...
        if not self.recording:
            timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
            filename = f"kinetic_timelapse_{timestamp}.mp4"  # Changed to MP4 for wider compatibility
            
            fourcc = cv2.VideoWriter_fourcc(*'mp4v')
            self.video_writer = cv2.VideoWriter(filename, fourcc, 20.0, (self.width, self.height)) # 20 FPS
            self.recording = True
    ...
    def clear_canvas(self):
        ...
        self.drawing_canvas = self.drawing_canvas.copy() # Ensure new object for undo
    ...
    def process_hands(self, frame):
        ...
        if hand_results.multi_hand_landmarks:
            for hand_idx, hand_landmarks in enumerate(hand_results.multi_hand_landmarks):
                ...
                # Check for swipe gesture for undo
                if hand_idx == 0:
    ...
    def run(self):
        ...
        try:
            while True:
                ...
                # Process hands and draw on the drawing_canvas
                frame = self.process_hands(frame)
                
                # Combine drawing layers with the frame
                display_frame = cv2.addWeighted(frame, 0.7, self.drawing_canvas, 0.3, 0)
                
                # Check for and handle icon touches
                all_extended_fingers = {}
                if self.hands.process(cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)).multi_hand_landmarks:
                    for hand_landmarks in self.hands.process(cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)).multi_hand_landmarks:
                        extended_fingers = self.get_extended_fingers(hand_landmarks)
                        for finger_name, pos in extended_fingers.items():
                            all_extended_fingers[f"{finger_name}"] = {'pos': pos, 'name': finger_name} # Simplified for icon check
                
                self.check_icon_touches(all_extended_fingers)
                
                # Draw icons on top
                self.draw_icons(display_frame)
                
                # Display information
                cv2.putText(display_frame, f"Brush: {self.brush_types[self.current_brush]}", (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 1)
                cv2.putText(display_frame, f"Pitch: {self.current_pitch:.0f}Hz", (10, 50), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 1)
                cv2.putText(display_frame, f"Volume: {self.current_volume:.1f}", (10, 70), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 1)
                cv2.putText(display_frame, f"Kaleidoscope: {'ON' if self.kaleidoscope_mode else 'OFF'}", (10, 90), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 1)
