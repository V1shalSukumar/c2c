...
class AdvancedKineticCanvas:
    ...
    def activate_button(self, button):
        ...
        if button.icon_type == "brush":
            ...
            if self.background_image is not None:
                # If a background is already loaded, check if it's currently enabled
                if self.background_enabled:
                    # If it's on, turn it off and let the user select a new one on the next click
                    self.background_enabled = False
                    print("Background turned off. Click again to load a new one.")
                else:
                    # If it's off, let the user select a new one
                    self.load_background_image_dialog()
            else:
                # If no background is loaded, open the dialog
                self.load_background_image_dialog()
    ...
    def draw_ui(self, frame):
        ...
        brush_name = self.brush_types[self.current_brush]
        cv2.putText(frame, f"Brush: {brush_name}", (10, y_offset_bottom),
