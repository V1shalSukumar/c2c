...
class AdvancedKineticCanvas:
    ...
    def apply_background_with_opacity(self, frame):
        """Apply background image to the frame with proper opacity control"""
        if not self.background_enabled or self.background_image is None:
            return frame
        
        # Create a weighted blend between the frame and background
        result = cv2.addWeighted(
            frame, 1.0 - self.background_opacity,  # Frame weight
            self.background_image, self.background_opacity,  # Background weight
            0
        )
    ...
    def save_image(self):
        ...
        if self.background_enabled and self.background_image is not None:
            # Apply background with proper opacity
            final_image = self.apply_background_with_opacity(final_image)
    ...
    def run(self):
        ...
        try:
            while True:
                ...
                # Use a copy for processing to avoid modifying the original frame too early
                ...
                # Create the final display frame with proper background handling
                display_frame = processed_frame.copy()
                
                # Apply background with opacity if enabled
                if self.background_enabled and self.background_image is not None:
                    display_frame = self.apply_background_with_opacity(display_frame)
                
                # Blend the drawing canvas onto the display frame
                drawing_overlay = self.drawing_canvas.copy()
                
                # Apply kaleidoscope effect if enabled
                drawing_overlay = self.apply_kaleidoscope_effect(drawing_overlay)
                
                # Create a mask for non-black pixels in the drawing overlay
                gray_drawing = cv2.cvtColor(drawing_overlay, cv2.COLOR_BGR2GRAY)
                _, mask = cv2.threshold(gray_drawing, 1, 255, cv2.THRESH_BINARY)
                mask_inv = cv2.bitwise_not(mask)
                
                # Blend drawing onto display frame
                display_frame_bg = cv2.bitwise_and(display_frame, display_frame, mask=mask_inv)
                drawing_fg = cv2.bitwise_and(drawing_overlay, drawing_overlay, mask=mask)
                final_display_frame = cv2.add(display_frame_bg, drawing_fg)

                # Check for and handle icon touches
                self.check_icon_touches(extended_fingers_all_hands)
                
                # Draw icons on top of everything else
                self.draw_icons(final_display_frame)
                
                # Draw other UI elements
                self.draw_ui(final_display_frame)
                
                # Record frame if recording
                if self.recording and self.video_writer:
                    self.video_writer.write(final_display_frame)
                
                cv2.imshow('Multi-Finger Kinetic Canvas with Background', final_display_frame)
