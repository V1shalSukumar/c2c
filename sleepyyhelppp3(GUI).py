...
class VirtualIcon:
    ...
    # The draw method is part of the frontend and has been removed.

class AdvancedKineticCanvas:
    """The backend engine that handles all logic, data, and image processing."""
    def __init__(self):
        ...
        self.menu_options = [
            ("Load BG", self.load_background_image_dialog),
            ("Exit Menu", self.toggle_menu)
        ]
        self.menu_touch_tracker = {}
        self.current_extended_fingers = {}

        print("Advanced Multi-Finger Kinetic Canvas Backend Initialized!")
    ...
    def is_two_fingers_up(self, hand_landmarks):
        if not hand_landmarks:
            return False
        index_extended = self.is_finger_extended(hand_landmarks, 'index')
        middle_extended = self.is_finger_extended(hand_landmarks, 'middle')
        ring_extended = self.is_finger_extended(hand_landmarks, 'ring')
        pinky_extended = self.is_finger_extended(hand_landmarks, 'pinky')
    ...
    def draw_smoke_brush(self, start_pos, end_pos, color, size, velocity):
        ...
        self.drawing_canvas = cv2.addWeighted(self.drawing_canvas, 1.0, overlay, 0.3, 0)
    ...
    def process_hands(self, frame, hand_results):
        ...
        if hand_results.multi_hand_landmarks:
            for hand_idx, hand_landmarks in enumerate(hand_results.multi_hand_landmarks):
                ...
                for finger_name, pos in extended_fingers.items():
                    ...
                    self.current_extended_fingers[finger_id] = {'pos': pos, 'name': finger_name}
                if hand_idx == 0:
                    self.detect_swipe_left(hand_landmarks)

        for finger_id, finger_data in self.current_extended_fingers.items():
        ...
        for finger_id, tracker in self.finger_trackers.items():
            if finger_id not in self.current_extended_fingers:
                if tracker['drawing_mode']:
        ...
        return self.current_extended_fingers
    ...
    def get_frame(self):
        """This method gets the latest frame from the camera, processes it, and returns it."""
        ...
        self.process_hands(processed_frame, hand_results)
        ...
        self.check_icon_touches(self.current_extended_fingers)
...
class KineticCanvasApp(App):
    """The Kivy frontend application."""
    def build(self):
        # Instantiate the backend engine
        self.kinetic_canvas_engine = AdvancedKineticCanvas()

        # Set up the main layout
        main_layout = BoxLayout(orientation='vertical')
        
        # Kivy Image widget to display the video feed
        ...
        # UI buttons that interact with the backend
        btn_brush = Button(text='Next Brush', font_size=20)
        btn_brush.bind(on_press=self.on_brush_button_press)
        button_layout.add_widget(btn_brush)
        
        btn_bg = Button(text='Load Background', font_size=20)
        btn_bg.bind(on_press=self.on_bg_button_press)
        button_layout.add_widget(btn_bg)
        
        btn_clear = Button(text='Clear', font_size=20)
        btn_clear.bind(on_press=self.on_clear_button_press)
        button_layout.add_widget(btn_clear)

        btn_save = Button(text='Save', font_size=20)
        btn_save.bind(on_press=self.on_save_button_press)
        button_layout.add_widget(btn_save)

        btn_record = Button(text='Record', font_size=20)
        btn_record.bind(on_press=self.on_record_button_press)
    ...
    def update_video(self, dt):
        """Called every frame to update the image widget."""
        frame = self.kinetic_canvas_engine.get_frame()
        if frame is not None:
            ...
            # Update the Kivy Image widget with the new texture
            self.image_widget.texture = image_texture

    def on_brush_button_press(self, instance):
        self.kinetic_canvas_engine.current_brush = (self.kinetic_canvas_engine.current_brush + 1) % len(self.kinetic_canvas_engine.brush_types)
        print(f"Brush changed to: {self.kinetic_canvas_engine.brush_types[self.kinetic_canvas_engine.current_brush]}")

    def on_bg_button_press(self, instance):
        self.kinetic_canvas_engine.load_background_image_dialog()

    def on_clear_button_press(self, instance):
        self.kinetic_canvas_engine.clear_canvas()

    def on_save_button_press(self, instance):
        self.kinetic_canvas_engine.save_image()

    def on_record_button_press(self, instance):
        if self.kinetic_canvas_engine.recording:
            self.kinetic_canvas_engine.stop_video_recording()
        else:
            self.kinetic_canvas_engine.start_video_recording()

if __name__ == '__main__':
    try:
        KineticCanvasApp().run()
    except Exception as e:
        print(f"Error: {e}")
